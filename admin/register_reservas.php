    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if($resultadoReservaExistente){
            ...
            if($cantidadLibrosReservado >= 3){
                header("Location: reservaAdmin.php?id=$idSesion&mensaje=Error: Ya tienes 3 libros reservados&error=5");
                exit;
            }
        ...
        if(isset($fechaReserva) && !empty($_POST['dateReserva'])){
            if($fechaReserva < $fechaActual){
                header("Location: reservaAdmin.php?id=$idSesion&mensaje=La fecha de reserva no puede ser menor&error=2");
            }else{
                ...
                if(mysqli_num_rows($resultadoReserva) > 0){
                    header("Location: reservaAdmin.php?id=$idSesion&mensaje=Ya tienes una reserva de este libro&error=4");
                    exit;
                }else{
                    ...
                    if($resultado){
                        ...
                        if($cantidad === -1){
                            header("Location: reservaAdmin.php?id=$idSesion&mensaje=No hay libros disponibles&error=3");
                            exit;
                        }else{
                            ...
                            if($resultadoUpdate){
                                header("Location: reservaAdmin.php?id=$idSesion&mensaje=Reserva exitosa&exito=1");
                                header("Refresh:0");
                            }
                        }
                    }
            ...
            header("Location: reservaAdmin.php?mensaje=La fecha de reserva es obligatoria&error=1");
        }
    }
?>